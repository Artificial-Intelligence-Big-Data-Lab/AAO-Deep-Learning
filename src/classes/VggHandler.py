...
class VggHandler:
    ...
    def __init__(self, iperparameters):
        ...
        label_df = datasets.get_label(freq='1d', thr=self.iperparameters['hold_labeling']).reset_index()
    ...
    def train_2D(self, training_set, validation_set, test_set, index_net, index_walk):  
        ...
        if self.iperparameters['multivariate'] == False:
            # CLASSICO UNIVARIATE
            #model = SmallerVGGNet.build_small_2d(height=self.input_shape[0], width=self.input_shape[1], depth=self.input_shape[2], init_var=index_net)
            model = SmallerVGGNet.build_alexNet_ternary(height=self.input_shape[0], width=self.input_shape[1], depth=self.input_shape[2])

        if self.iperparameters['multivariate'] == True:
            # MULTIVARIATE
            model = SmallerVGGNet.build_parallel_2d(width=self.iperparameters['input_shape_single_signal'][0], 
                                                    height=self.iperparameters['input_shape_single_signal'][1], 
                                                    depth=self.input_shape[2], 
                                                    init_var=index_net)

        #model = SmallerVGGNet.build_vgg16_2d_smaller(height=self.input_shape[0], width=self.input_shape[1], depth=self.input_shape[2], init_var=index_net, classes=3)
        ...
        #filepath = self.output_folder + 'models/epochs/' +"weights-improvement-{epoch:02d}-{val_acc:.2f}.hdf5"
        ...
        if self.save_model_history == True: 
            ...
            H = model.fit(  x=training_set.get_x(normalized=True, multivariate=self.iperparameters['multivariate']), 
                            ...
                            batch_size=self.bs, 
                            validation_data=(validation_set.get_x(normalized=False, multivariate=self.iperparameters['multivariate']), validation_set.get_y(type="categorical", referred_to='next_day')), 
                            epochs=self.epochs, 
                            verbose=1, 
        ...
        if self.save_model_history == False:
            ...
            H = model.fit(  x=training_set.get_x(normalized=True, multivariate=self.iperparameters['multivariate']), 
                            y=training_set.get_y(type="categorical", referred_to='next_day'), 
                            batch_size=self.bs, 
                            validation_data=(validation_set.get_x(normalized=False, multivariate=self.iperparameters['multivariate']), validation_set.get_y(type="categorical")), 
                            epochs=self.epochs, 
                            verbose=1, 
                            callbacks=callbacks_list)#,
    ...
    def get_train(self, training_set):
        ...
        for index, input_dataset in enumerate(self.input_datasets):
            ...
            label_df = market.get_label(freq='1d', columns=['delta_next_day', 'delta_current_day', 'close'], thr=self.iperparameters['hold_labeling']).reset_index()
        ...
        x = np.zeros(shape=(df.shape[0], self.input_shape[0], self.input_shape[1], self.input_shape[2]), dtype=np.uint8)
        ...
        set = Set(date_time=date_list, x=x, y_current_day=y_current_day, y_next_day=y_next_day, delta_current_day=delta_current_day, delta_next_day=delta_next_day, close=close)
    ...
    def get_set_predictions_dataset(self, date_set):   
        ...
        df = market.get_label(freq='1d', columns=['delta_next_day', 'delta_current_day', 'open', 'close'], thr=self.iperparameters['hold_labeling']).reset_index()
        ...
        x = np.zeros(shape=(len(date_list), self.input_shape[0], self.input_shape[1], self.input_shape[2]), dtype=np.uint8)
    ...
    def run_2D(self, start_index_walk=None, gpu_id=0):
        ...
        for index_walk in range(self.number_of_walks):
            ...
            for index_net in range(self.number_of_nets):
                ...
                #test_accuracy = 0
                #results = np.zeros(shape=(test_size), dtype=np.uint8)
