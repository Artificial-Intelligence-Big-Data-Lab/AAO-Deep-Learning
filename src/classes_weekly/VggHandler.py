...
class VggHandler:
    ...
    if platform == 'Linux-4.15.0-45-generic-x86_64-with-Ubuntu-16.04-xenial': 
        ...
        output_base_path = '../experiments/' # locale 
    ...
    def train_2D(self, training_set, validation_set, test_set, index_net, index_walk):  
        ...
        if self.save_model_history == True: 
            ...
            H = model.fit(  x=training_set.get_x(normalized=True), 
                            y=training_set.get_y(type="categorical", referred_to='next_day'), 
                            batch_size=self.bs, 
                            validation_data=(validation_set.get_x(normalized=False), validation_set.get_y(type="categorical", referred_to='next_day')), 
                            epochs=self.epochs, 
                            verbose=1, 
        ...
        if self.save_model_history == False:
            ...
            H = model.fit(  x=training_set.get_x(normalized=True), 
                            y=training_set.get_y(type="categorical", referred_to='next_day'), 
                            batch_size=self.bs, 
                            validation_data=(validation_set.get_x(normalized=False), validation_set.get_y(type="categorical")), 
                            epochs=self.epochs, 
                            verbose=1, 
                            callbacks=callbacks_list)#,
        ...
        df_loss['epoch'] = range(1, len(H.history['loss']) + 1)

        df_loss['training_loss'] = H.history['loss']
        df_loss['validation_loss'] = H.history['val_loss']
        df_loss['test_loss'] = H.history['test_loss']
        
        path = self.output_folder + 'predictions/loss_during_training/' + 'walk_' + str(index_walk) + '/' 
        filename = path + 'net_' + str(index_net) + '.csv'
        create_folder(path)
        df_loss.to_csv(filename, header=True, index=False)
    ...
    def get_train(self, training_set):
        ...
        for index, input_dataset in enumerate(self.input_datasets):
            ...
            images_list_df['images_path'] = self.images_full_paths[index] + images_list_df['date_time'] + '.png'
    ...
    def get_set_predictions_dataset(self, date_set):   
        ...
        for i in range(len(date_list)):
            x[i] = cv2.imread(self.images_base_path + self.predictions_images_folder + date_list[i].strftime('%Y-%m-%d') + '.png')
