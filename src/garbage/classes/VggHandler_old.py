...
class VggHandler:
    ...
    def __train_2D(self, x_train, y_train, x_val, y_val, index_net, index_walk):
        ...
        if self.__save_model_history == True: 
            ...
            H = model.fit(x=X_train, y=y_train, batch_size=self.__bs, validation_data=(x_val, y_val), epochs=self.__epochs, verbose=1)#, sample_weight=weights)
    ...
    def run_2D(self):
        ...
        for index_walk in range(self.__number_of_walks):
            ...
            x_train, y_train, x_val, y_val, x_test, y_test = self.__get_train_val_test(training_set=self.__training_set[index_walk],
            ...
            for index_net in range(self.__number_of_nets):
                ...
                model, H = self.__train_2D(x_train=x_train, y_train=y_train, x_val=x_val, y_val=y_val, index_net=index_net, index_walk=index_walk)
    ...
    def get_predictions_2D(self, set_type):
        ...
        for index_walk in range(0, self.__number_of_walks):
            ...
            x_train, y_train, x_val, y_val, x_test, y_test = self.__get_train_val_test(
