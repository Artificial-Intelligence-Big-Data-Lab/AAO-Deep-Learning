...
class VggHandler:
    ...
    def __save_plots(self, H, index_walk, index_net):
        ...
        plt.subplot(2, 1, 1)
        plt.plot(np.arange(0, self.__epochs), H.history["metrica"], label="spc acc")
        plt.plot(np.arange(0, self.__epochs), H.history["accuracy"], label="acc")
        #plt.plot(np.arange(0, self.__epochs), H.history["accuracy"], label="train_acc")
        #plt.plot(np.arange(0, self.__epochs), H.history["val_accuracy"], label="val_acc")
    ...
    '''
    def __train_2D(self, x_train, y_train, delta_train, x_val, y_val, index_net, index_walk):
        # Carico le immagini
        X_train = x_train.astype('float32') / 255
        X_val = x_val.astype('float32') / 255

        #print([int(i) for i in y_train])
        ...
        if self.__save_model_history == True: 
            ...
            H = model.fit(x=X_train, y=y_train, batch_size=self.__bs, validation_data=(x_val, y_val), epochs=self.__epochs, verbose=1, callbacks=[LossAndErrorPrintingCallback()]) #class_weight=class_weight)
       
        # Non voglio salvare la storia dei modelli ogni tot epoche, salvo solo l'ultimo
        if self.__save_model_history == False:
            H = model.fit(X_train, y_train, batch_size=self.__bs, validation_data=(x_val, y_val), epochs=self.__epochs, verbose=1, sample_weight=weights)
    ...
    def __get_train_val_test(self, training_set, validation_set, test_set, df_label_type):
        ...
        x_train, y_train, delta_train = self.__get_train(training_set=training_set, df_label_type=df_label_type)
        
        # calcolo validation set prendendo come samples solo il mercato di riferimento
        x_val, y_val, delta_val = self.__get_set_predictions_dataset(date_set=validation_set, df_label_type=df_label_type)

        # calcolo il test set 
        x_test, y_test, delta_test = self.__get_set_predictions_dataset(date_set=test_set, df_label_type=df_label_type)
        
        return x_train, y_train, delta_train,  x_val, y_val, delta_val, x_test, y_test, delta_test
    ...
    def run_2D(self):
        ...
        for index_walk in range(self.__number_of_walks):
            ...
            for index_net in range(self.__number_of_nets):
                ...
                model, H = self.__train_2D(x_train=x_train, y_train=y_train, delta_train=delta_train, x_val=x_val, y_val=y_val, index_net=index_net, index_walk=index_walk)
